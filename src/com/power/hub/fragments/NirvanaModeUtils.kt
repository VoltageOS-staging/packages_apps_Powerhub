...
/**
 * Enterprise-Grade Helper for Nirvana Mode logic.
 * Features: Idempotent State Reconciliation, Multi-User Support, Transaction Logging.
 */
class NirvanaModeUtils(private val context: Context) {
    ...
    companion object {
        ...
        private const val KEY_APPS = "nirvana_mode_apps_list" // Selected packages (raw)
        private const val KEY_TRACKED_STATE = "nirvana_mode_tracked_state" // "userId|pkg" of currently suspended apps
        ...
        // Defaults
        private const val DEFAULT_START = 540 // 09:00
        private const val DEFAULT_END = 1020 // 17:00
    }
    ...
    */
    ...
    private fun applyBatchSuspension(pkgs: Array<String>, suspend: Boolean, userId: Int, dialogInfo: SuspendDialogInfo?) {
        try {
            val finalPkgs = if (suspend) {
    ...
    private fun isPackageSuspendedForUser(pkg: String, userId: Int): Boolean {
        return try {
    ...
    private fun isSuspendedByMe(pkg: String, userId: Int): Boolean {
        return try {
    ...
    private fun getStrictDialogInfo(): SuspendDialogInfo {
        val builder = SuspendDialogInfo.Builder()
    ...
    private fun getNextOccurrence(now: Calendar, targetMinutes: Int, currentMinutes: Int): Long {
